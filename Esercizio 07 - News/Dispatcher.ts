...
dispatcher.addListener("GET", "elenco", function (req, res) {
    res.writeHead(200, HEADERS.json);
    res.write(JSON.stringify(news));
    res.end();
});

dispatcher.addListener("POST", "dettagli", function (req, res) {
    let nomeFile = req["BODY"].file;
    _fs.readFile(`./news/${nomeFile}`, function (err, testo) {
        if (!err)
        {
            let aus;
            for(let vis of news)
                if(vis.file == nomeFile)
                {
                    vis.visualizzazioni++;
                    aus = vis;
                }
            ...
            res.writeHead(200, HEADERS.json);
            res.write(JSON.stringify({ "testo" : testo.toString(), "vis" : aus.visualizzazioni }));
            res.end();
        }
        else
        {
            res.writeHead(404, HEADERS.json);
            res.write('{ "err" : "File non trovato" }');
            res.end();
