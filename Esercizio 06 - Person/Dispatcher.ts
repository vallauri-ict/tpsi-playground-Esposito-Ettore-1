...
dispatcher.addListener("GET", "nazioni", function (req, res) {
    let nazioni = [];
    for(let pers of persons.results)
    {
        let naz = pers.location.country;
        if (!nazioni.includes(naz))
            nazioni.push(naz);
    }

    res.writeHead(200, HEADERS.json);
    res.write(`{"nazioni" : "${nazioni}"}`);
    res.end();
});

dispatcher.addListener("GET", "persone", function (req, res) {
    let nazioni = [];
    for(let pers of persons.results)
    {
        let naz = pers.location.country;
