...
dispatcher.addListener("POST", "radios", function (req, res) {
    let ris :any = [];
    ...
    {
        res.writeHead(200, HEADERS.json);
        res.write(JSON.stringify(ris));
        res.end();
    }
    else
    {
        res.writeHead(404, HEADERS.json);
        res.write(`{ "err" : "No radio found for given state : ${state}" }`);
        res.end();
    }
});

dispatcher.addListener("POST", "like", function (req, res) {
    let id = req["BODY"].id;
    ...
    {
        res.writeHead(404, HEADERS.json);
        res.write(`{ "err" : "No radio found for given id : ${id}" }`);
        res.end();
    }
});